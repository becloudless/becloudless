...
func NixosHardwareDisksCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:     "disks [name|path|device]...",
		Aliases: []string{"disk"},
		Short:   "disks related commands",
		Long: "List bcl.disks entries, as configured in /etc/bcl/config.yaml, along with their status, path and per-device location.\n" +
			"If one or more arguments are given, only disks matching at least one of them are printed.\n" +
			"Each argument can be a disk name, a mount point, a path within a mount point, or a block\n" +
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			filterDiskNames := map[string]bool{}
			filterPhysicalNames := map[string]bool{}
			for _, arg := range args {
				diskName, physicalNames, err := resolveArgFilter(disks, run, graph, arg)
				if err != nil {
					return err
				}
				if diskName != "" {
					filterDiskNames[diskName] = true
				}
				for _, name := range physicalNames {
					filterPhysicalNames[name] = true
				}
			}
			...
			for _, diskName := range physicalDiskNames {
				...
				if len(args) > 0 && !filterPhysicalNames[diskName] && !filterDiskNames[info.Name] {
					continue
				}
			...
			for _, name := range missingNames {
				if len(args) > 0 && !filterDiskNames[name] {
					continue
				}
...
// resolveArgFilter resolves a single argument passed to the disks command
// (a disk name, a mount point, a path within a mount point, or a block
// device - a raw disk, partition, LUKS mapper, raid device, a bare kernel
// name as reported by dmesg such as "sda", a SCSI H:C:T:L address such as
// "7:0:29:0", or a /dev/disk/by-id|by-uuid|by-path link) down to the
// identifiers used to filter the disk list: the config disk name (if arg is
// one) and/or the resolved physical device name(s) (e.g. /dev/nvme0n1).
func resolveArgFilter(disks map[string][]bcl.DeviceConfig, run runner.Runner, graph *blockDeviceGraph, arg string) (string, []string, error) {
	if devices, ok := disks[arg]; ok {
		var physicalNames []string
		for _, device := range devices {
			if node, found := graph.find(run, device.Path); found {
				physicalNames = append(physicalNames, physicalDeviceNames(graph.relatedChain(node.Name))...)
			}
		}
		return arg, physicalNames, nil
	}

	if node, found := graph.find(run, arg); found {
		// arg is a block device (raw disk, partition, mapper, raid device,
		// or a link to one): find its physical disk(s) directly from the
		// system's block device tree.
		return "", physicalDeviceNames(graph.relatedChain(node.Name)), nil
	}

	// Not a recognized disk name or block device: treat arg as a mount
	// point, or an arbitrary path within one.
	out, err := run.ExecCmdGetStdout("findmnt", "-n", "-o", "TARGET", "--target", arg)
	if err != nil {
		return "", nil, errs.WithEF(err, data.WithField("arg", arg), "Failed to resolve mount point")
	}
	...
	// btrfs subvolumes (e.g. "/dev/mapper/nix[/home/user]"); strip it.
