...
// mergeEnabledTransform merges the well-known "enabled" field (handled by
...
func mergeEnabledTransform(schema map[string]interface{}, _ *entry) (map[string]interface{}, error) {
	mergedProps := map[string]interface{}{}
	if p, ok := schema["properties"].(map[string]interface{}); ok {
		for k, v := range p {
			mergedProps[k] = v
