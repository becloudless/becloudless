...
#      default = lib.mkIf cfg.ssh "";
  ...
  config = lib.mkIf cfg.enable {
    assertions = [
      {
        assertion = !(cfg.ssh && cfg.loader == "uboot");
        message = "bcl.boot.ssh and bcl.boot.loader = \"uboot\" cannot be used together. The generic-extlinux-compatible bootloader does not support initrd secrets required for SSH host keys.";
      }
    ];
...
#          theme = "${pkgs.grub-cyberexs}/share/grub/themes/CyberEXS";
      ...
      initrd = {
        ...
        network.ssh = lib.mkIf cfg.ssh {
          ...
          hostKeys = mkIf (cfg.loader != "uboot") ["/etc/ssh/initrd_ssh_host_ed25519_key" ];
        };
