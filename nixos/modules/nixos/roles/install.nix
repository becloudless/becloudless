{ config, lib, pkgs, ... }: {
  ...
  config = lib.mkIf (config.bcl.role.name == "install") {
    ...
    environment.systemPackages = with pkgs; [
      git
      (writeShellScriptBin "bcl-install" ''
        #/usr/bin/env bash
        set -e

        function echo_purple {	echo -e -n "\e[0;35m$1\e[0m" ; echo "$2";}

        if [ ! -d infra ]; then
          echo_purple "Cloning infra repo"
          git clone git@gitea.bcl.io:bcl/infra.git
        else
          git -C infra pull
        fi
        sudo --preserve-env=SSH_AUTH_SOCK infra/bin/nixos-install.sh $@
      '')
    ];
