...
let
  ...
  clusterNumber = lib.strings.toInt(builtins.substring ((builtins.stringLength config.networking.hostName) -2) 1 config.networking.hostName);
in
{
  ...
  config = lib.mkIf (config.bcl.role.name == "srv") {
    ...
    systemd.network.netdevs.br40.netdevConfig = {
      Kind = "bridge";
      Name = "br40";
    };
    systemd.network.networks.br40 = {
      matchConfig = {
        Name = "br40";
      };
      networkConfig = {
    ...
    networking.extraHosts = ''
      127.0.0.1       srv${toString clusterNumber}${toString srvNumber}.h.bcl.io ${config.networking.hostName} localhost
      127.0.0.1       k${toString clusterNumber}.bcl.io
    ...
    environment.etc."kubernetes/kubeadm.yaml".text = ''
        ...
        clusterName: bcl
        controlPlaneEndpoint: k${toString clusterNumber}.bcl.io:8443
        networking:
          serviceSubnet: 172.16.4${toString clusterNumber}.0/24
          podSubnet: 10.4${toString clusterNumber}.0.0/16
        etcd:
          local:
            serverCertSANs:
        ...
        apiServer:
          ...
          certSANs:
          - k${toString clusterNumber}.bcl.io
          - k${toString clusterNumber}.h.bcl.io
          - 1.bcl.io
          - 2.bcl.io
