...

{
  config = lib.mkIf (config.bcl.role.name == "srv") {
      ...
      mkRegistry = name: {
        ...
        value = {
          text = ''
            ...
            # location = "docker.bcl.io"    # replace location with mirror. mirror is mandatory

            [[registry.mirror]]
            location = "docker.bcl.io"
          '';
        };
      };
    in builtins.listToAttrs [
      ...
      (mkRegistry "docker.bcl.io")
      (mkRegistry "registry.bcl.io")
      (mkRegistry "quay.io")
      (mkRegistry "ghcr.io")
      (mkRegistry "tccr.io") # truecharts
      (mkRegistry "nvcr.io") # nvidia
      {
        name = "crio/crio.conf.d/bcl.crio.conf";
        value = {
          text = ''
             [crio.network]
