...
in
{
  ...
  config = lib.mkIf (cfg.vms != {}) {
    ...
    # Even with the above, lvm-activate-data.service (generated by
    # lvm2-activation-generator, triggered via udev the instant the "data"
    # VG's LUKS device appears) can win a boot-order race against /nix itself
    # being mounted, when /nix is a SEPARATE LUKS-encrypted device from
    # "data" (as on srv3). thin_check_executable points into the /nix store,
    # so if lvm-activate-data.service runs before /nix is mounted, thin_check
    # is unreachable and activation fails with the exact same
    # "status=5/NOTINSTALLED" symptom as if thin.enable were never set -
    # leaving VM root disks (thin LVs) inactive after every reboot. Confirmed
    # on srv3 (2026-09-16): lvm-activate-data.service ran/failed 2s BEFORE
    # nix.mount became active. Force it to wait for /nix to be mounted
    # (merged into the systemd.services set built further below, since Nix
    # module config attrsets can't define the same top-level attribute twice).
    ...
    systemd.services = (lib.mapAttrs' (name: vm:
      ...
      # See the "lvm-activate-data.service" comment above services.udev.extraRules:
      # force this generator-created unit to wait for /nix to be mounted, since
      # thin_check_executable (from services.lvm.boot.thin.enable) lives in the
      # /nix store. A NixOS systemd.services override creates a drop-in for this
      # unit name regardless of it being generator-produced rather than
      # package/module-declared, so this reliably applies.
      lvm-activate-data = {
        after = [ "nix.mount" ];
        requires = [ "nix.mount" ];
      };
